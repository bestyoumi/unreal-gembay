...
/** FGemBaySkillsAction */
FText FGemBaySkillsAction::GetDisplayName() const { return LOCTEXT("SkillsActionName", "Installed Agent Skills"); }

void FGemBaySkillsAction::Initialize() { RefreshSkills(); }

void FGemBaySkillsAction::RefreshSkills()
{
	...
	{
		FGemBaySkillInfo Info; Info.Name = Folder;
		...
		if (FFileHelper::LoadFileToString(Content, *SkillPath))
		{
			...
			if (DescStart != INDEX_NONE)
			{
				...
				if (LineEnd != INDEX_NONE) { Info.Description = Content.Mid(DescStart, LineEnd - DescStart).TrimStartAndEnd(); }
			}
		}
...
TSharedRef<SWidget> FGemBaySkillsAction::GetWidget()
{
	RefreshSkills();
	TSharedRef<SVerticalBox> Widget = SNew(SVerticalBox);
	Widget->AddSlot().AutoHeight().Padding(10) [ SNew(STextBlock).Text(GetDisplayName()).Font(FCoreStyle::GetDefaultFontStyle("Bold", 14)) ];

	for (const auto& Skill : AvailableSkills)
	{
		Widget->AddSlot().AutoHeight().Padding(5, 5) [
			SNew(SBorder).Padding(10).BorderImage(FCoreStyle::Get().GetBrush("ToolPanel.GroupBorder"))
			[
				SNew(SVerticalBox)
				+ SVerticalBox::Slot().AutoHeight() [ SNew(STextBlock).Text(FText::FromString(Skill.Name)).Font(FCoreStyle::GetDefaultFontStyle("Bold", 10)) ]
				+ SVerticalBox::Slot().AutoHeight().Padding(0, 5, 0, 0) [ SNew(STextBlock).Text(FText::FromString(Skill.Description)).AutoWrapText(true) ]
			]
		];
	}
	return Widget;
...
TSharedRef<SWidget> FGemBayMaterialAIAction::GetWidget()
{
	...
	return SNew(SVerticalBox)
		+ SVerticalBox::Slot().AutoHeight().Padding(0, 20, 0, 10) [ SNew(STextBlock).Text(GetDisplayName()).Font(FCoreStyle::GetDefaultFontStyle("Bold", 14)) ]
		+ SVerticalBox::Slot().AutoHeight().Padding(5, 5) [
			SNew(SBorder).Padding(10).BorderImage(FCoreStyle::Get().GetBrush("ToolPanel.GroupBorder"))
			[
				SNew(SVerticalBox)
				+ SVerticalBox::Slot().AutoHeight() [ SNew(STextBlock).Text(LOCTEXT("AIInputLabel", "Describe the material you want to create:")).Font(FCoreStyle::GetDefaultFontStyle("Regular", 9)) ]
				+ SVerticalBox::Slot().AutoHeight().Padding(0, 5, 0, 10) [ SAssignNew(MaterialDescBox, SEditableTextBox).HintText(LOCTEXT("AIHint", "e.g., a glowing translucent blue glass with high refraction")) ]
		...
		+ SVerticalBox::Slot().AutoHeight().Padding(0, 20, 0, 10) [ SNew(STextBlock).Text(LOCTEXT("InteropsHeader", "Materials System Interops")).Font(FCoreStyle::GetDefaultFontStyle("Bold", 14)) ]
		+ SVerticalBox::Slot().AutoHeight().Padding(5, 5) [
			SNew(SBorder).Padding(10).BorderImage(FCoreStyle::Get().GetBrush("ToolPanel.GroupBorder"))
			[
				SNew(SHorizontalBox)
		...
		+ SVerticalBox::Slot().AutoHeight().Padding(5, 5) [
			...
			[
				...
				+ SVerticalBox::Slot().AutoHeight() [
					SNew(SHorizontalBox)
					+ SHorizontalBox::Slot().FillWidth(1.0f) [ SNew(STextBlock).Text(LOCTEXT("BridgeDesc", "Run scripts rapidly in the current process.")).Font(FCoreStyle::GetDefaultFontStyle("Italic", 8)) ]
